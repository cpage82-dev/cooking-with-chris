  ...
  const fetchRecipe = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        recipe_image: null, // Will be set if user uploads new image
        current_image_url: data.recipe_image || '',
        course_type: data.course_type,
        recipe_type: data.recipe_type,
        primary_protein: data.primary_protein,
        ethnic_style: data.ethnic_style,
        prep_time: data.prep_time,
        cook_time: data.cook_time,
        number_servings: data.number_servings,
      });

      // Populate ingredient sections (remove 'id' field for update)
      setIngredientSections(
        data.ingredient_sections.map(section => ({
          section_title: section.section_title,
          section_order: section.section_order,
          ingredients: section.ingredients.map(ing => ({
            ingredient_quantity: ing.ingredient_quantity || '',
            ingredient_uom: ing.ingredient_uom || '',
      ...
      // Populate instruction sections (remove 'id' field for update)
      setInstructionSections(
        data.instruction_sections.map(section => ({
          section_title: section.section_title,
          section_order: section.section_order,
          instructions: section.instructions.map(inst => ({
            instruction_step: inst.instruction_step,
            step_order: inst.step_order,
          })),
        }))
      );
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="space-y-4">
            ...
            {formData.current_image_url && !formData.recipe_image && (
              <div>
                <label className="block text-gray-700 font-medium mb-2">
                  Current Image
                </label>
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-2">
                {formData.current_image_url ? 'Replace Image (Optional)' : 'Recipe Image (Optional)'}
              </label>
              <input
                type="file"
                accept="image/*"
              ...
              {formData.recipe_image && (
                <p className="text-sm text-green-600 mt-1">
                  New image selected: {formData.recipe_image.name}
                </p>
              )}
            </div>
